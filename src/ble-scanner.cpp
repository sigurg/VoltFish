...
void BLEScanner::addDevice(const QBluetoothDeviceInfo &info) {
    ...
    static const QBluetoothUuid METER_SERVICE(
        QString("1bc5ffa0-0200-62ab-e411-f254e005dbd4"));

    if (info.coreConfigurations() &
            QBluetoothDeviceInfo::LowEnergyCoreConfiguration) 	// is BLE device
        if (info.majorDeviceClass() ==
                QBluetoothDeviceInfo::MiscellaneousDevice)		// misc device
            if (info.serviceUuids().contains(
                        METER_SERVICE)) {							// has meter service UUID
