...
public:
    ...
    Q_PROPERTY(Mapping ch1_mapping MEMBER ch1_mapping WRITE set_ch1_mapping NOTIFY
               ch1Config);
    Q_PROPERTY(Analysis ch1_analysis MEMBER ch1_analysis WRITE set_ch1_analysis
               NOTIFY ch1Config);
    Q_PROPERTY(int ch1_range MEMBER ch1_range WRITE set_ch1_range NOTIFY
               ch1Config);

    Q_PROPERTY(Mapping ch2_mapping MEMBER ch2_mapping WRITE set_ch2_mapping NOTIFY
               ch2Config);
    Q_PROPERTY(Analysis ch2_analysis MEMBER ch2_analysis WRITE set_ch2_analysis
               NOTIFY ch2Config);
    Q_PROPERTY(int ch2_range MEMBER ch2_range WRITE set_ch2_range NOTIFY
               ch2Config);

    Q_PROPERTY(MathMode math_mode MEMBER math_mode WRITE set_math_mode NOTIFY
               mathConfig);
    ...
    Q_PROPERTY(QStringList model_ch1_range MEMBER model_ch1_range NOTIFY
               ch1modelChanged);
    Q_PROPERTY(QStringList model_ch2_range MEMBER model_ch2_range NOTIFY
               ch2modelChanged);

    Q_PROPERTY(QString btaddr MEMBER btaddr WRITE set_btaddr NOTIFY btaddrChanged);
    Q_PROPERTY(TempUnit temp_unit MEMBER temp_unit WRITE set_temp_unit NOTIFY
               tempunitChanged);

    Q_INVOKABLE QStringList model_rate() {
        return valid_rates;
    };
...
private:
    ...
    bool is_out_of_range(const Mapping &mapping, const int &range,
                         const double &val);

    void set_ch1();
    void set_ch2();

    void channel_config(const int &channel, Mapping &mapping, Analysis &analysis,
                        int &range);
